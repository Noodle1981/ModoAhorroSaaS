        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <div class="mb-6">
                    ...
                    @php
                        $plotArea = is_array($entity->details ?? null) ? ($entity->details['area_m2'] ?? null) : null;
                        $suggestedRoof = $plotArea ? round($plotArea * \App\Services\SolarPotentialAnalysisService::FALLBACK_ROOF_RATIO_FROM_PLOT, 1) : null;
                    @endphp
                    @if(!$entity->roof_area_m2 && !$entity->ground_area_m2 && $plotArea)
                        <div class="mt-4 bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
                            <div class="flex items-start">
                                <i class="fas fa-ruler-combined text-yellow-600 text-xl mr-3 mt-1"></i>
                                <div>
                                    <p class="text-sm text-yellow-900">
                                        Tu entidad declaró <strong>{{ number_format($plotArea, 0) }} m²</strong> de área de lote. Podemos usar una regla rápida para estimar el techo disponible: <strong>{{ number_format($suggestedRoof, 1) }} m²</strong> (≈20% del lote).
                                    </p>
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        <button type="button"
                                            onclick="(function(){var el=document.getElementById('roof_area_m2'); if(el){ el.value='{{ $suggestedRoof }}'; el.focus(); } })()"
                                            class="inline-flex items-center px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 text-white rounded-md text-sm">
                                            <i class="fas fa-magic mr-2"></i> Autocompletar techo con {{ number_format($suggestedRoof, 1) }} m²
                                        </button>
                                        <button type="button"
                                            onclick="(function(){var el=document.getElementById('ground_area_m2'); if(el){ el.value='{{ $suggestedRoof }}'; el.focus(); } })()"
                                            class="inline-flex items-center px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-md text-sm">
                                            <i class="fas fa-seedling mr-2"></i> Usar como terreno ({{ number_format($suggestedRoof, 1) }} m²)
                                        </button>
                                    </div>
                                    <p class="mt-2 text-xs text-yellow-800">
                                        Es una referencia inicial para avanzar rápido. Si conocés la medida real del techo o del patio, ingresala para mayor precisión.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <form action="{{ route('solar.configure.store', $entity) }}" method="POST" class="space-y-8">
                    @csrf
