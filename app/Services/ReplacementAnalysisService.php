...
use App\Models\MissingMarketAlternative;
...
public function findReplacementOpportunities(Collection $inventoryWithCalculations, $costoUnitarioKwh)
{
    ...
    foreach ($inventoryWithCalculations as $userEquipment) {
        ...
        $currentPowerWatts = $userEquipment->power_watts_override ?? $userEquipment->equipmentType->default_power_watts;
        ...
        $potentialReplacements = MarketEquipmentCatalog::where('equipment_type_id', $userEquipment->equipment_type_id)
            ->where('power_watts', '<', $currentPowerWatts)
            ->get();

        if ($potentialReplacements->isEmpty()) {
            MissingMarketAlternative::updateOrCreate(
                ['equipment_type_id' => $userEquipment->equipment_type_id],
        ...
        $energiaActualKwh = $userEquipment->consumo_kwh_activo_periodo;
        ...
        $avgDailyUseMinutes = $userEquipment->avg_daily_use_minutes_override ?? $userEquipment->equipmentType->default_avg_daily_use_minutes ?? 0;
        $horasDeUsoAnual = ($avgDailyUseMinutes / 60) * 365;

        $consumoNominalNuevoKW = $bestReplacement->power_watts / 1000;
        $energiaNuevaKwh = ($horasDeUsoAnual * $userEquipment->equipmentType->equipmentCategory->calculationFactor->load_factor * $userEquipment->quantity * $consumoNominalNuevoKW) / $userEquipment->equipmentType->equipmentCategory->calculationFactor->efficiency_factor;
        
        $ahorroAnualKwh = $energiaActualKwh - $energiaNuevaKwh;
        $ahorroAnualPesos = $ahorroAnualKwh * $costoUnitarioKwh;

        if ($ahorroAnualPesos > 0 && $bestReplacement->average_price > 0) {
            ...
            $opportunities[] = [
                ...
                'new_equipment_details' => $bestReplacement, // Añadimos detalles del nuevo equipo
            ];
        }
    }
